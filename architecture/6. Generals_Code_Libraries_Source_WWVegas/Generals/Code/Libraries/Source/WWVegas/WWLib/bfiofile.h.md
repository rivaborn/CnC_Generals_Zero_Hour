# Generals/Code/Libraries/Source/WWVegas/WWLib/bfiofile.h
...
## Globals
- **IsAllocated (bool)**: Tracks buffer allocation state.
- **IsOpen (bool)**: Tracks file open state.
- **Buffer (void*)**: Pointer to buffer memory.
- **BufferSize (long)**: Current buffer size.
- **FilePos (long)**: Current file position.

## Dependencies
- **rawfile.h**: Parent class `RawFileClass`.
- **READ/SEEK_CUR**: Constants from parent class.

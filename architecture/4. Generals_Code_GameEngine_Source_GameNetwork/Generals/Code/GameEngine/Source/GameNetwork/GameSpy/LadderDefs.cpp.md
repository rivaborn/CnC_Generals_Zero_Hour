# Generals/Code/GameEngine/Source/GameNetwork/GameSpy/LadderDefs.cpp

## Purpose
Parses and manages ladder (ranked match) definitions for the GameSpy network integration in Command & Conquer Generals.
...
- **LadderInfoList (typedef)**: List of LadderInfo pointers

## Key Functions
### parseLadder
- Purpose: Parses a single ladder definition from raw text
- Calls: `NEW`, `MultiByteToWideCharSingleLine`, `ThePlayerTemplateStore->getAllSideStrings`, `TheGameState->portableMapPathToRealMapPath`, `TheGameState->realMapPathToPortableMapPath`, `TheMapCache->findMap`, `TheGameSpyConfig->getQMMaps`
...
- External symbols: `TheGameSpyConfig`, `ThePlayerTemplateStore`, `TheGameState`, `TheMapCache`, `TheGlobalData`, `TheFileSystem`, `NEW`, `DEBUG_LOG`, `MultiByteToWideCharSingleLine`

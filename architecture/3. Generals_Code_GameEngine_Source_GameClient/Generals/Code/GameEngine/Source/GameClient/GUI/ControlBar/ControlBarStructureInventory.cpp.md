# Generals/Code/GameEngine/Source/GameClient/GUI/ControlBar/ControlBarStructureInventory.cpp
...
- Purpose: Initializes and populates all inventory buttons for a structure
- Calls: `resetContainData`, `findCommandButton`, `setControlCommand`, `contain->iterateContained`

### `updateContextStructureInventory`
- Purpose: Updates the inventory display when the selected object changes
- Calls: `TheInGameUI->deselectDrawable`, `populateStructureInventory`

## Globals
- **STOP_ID (const int)**: ID for stop command button
- **EVACUATE_ID (const int)**: ID for evacuate command button

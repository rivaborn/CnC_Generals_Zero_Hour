...
## Responsibilities
- Defines the `HmdlNodeDefStruct` for associating render objects with pivot/bones.
- Implements `HModelDefClass` as a blueprint for creating hierarchy models.
- Handles loading of W3D hierarchy model definitions.
- Manages sub-objects, snap points, and model metadata.

## Key Types
- `HmdlNodeDefStruct` (struct): Associates a named render object with an indexed pivot/bone.
- `HModelDefClass` (class): Blueprint for hierarchy models, storing model metadata and sub-objects.

...
#
# Usage:
#   .\arch_overview_local.ps1 [-TargetDir <dir>] [-Chunked] [-Single] [-Clean] [-Full]
# ============================================================
...
function Build-SummaryData($docRoot) {
    $docs = @(Get-PerFileDocs $docRoot)
    $sb = [System.Text.StringBuilder]::new()
    foreach ($doc in $docs) {
    ...
    return @{ Text = $sb.ToString(); Count = $docs.Count }
}
...
"@
    ...
    $subsystems = @(Get-Subsystems $docRoot '' $chunkThreshold 0)
    ...
    foreach ($sub in $subsystems) {
        ...
        if ($subData['Count'] -eq 0) { continue }

        Write-Host "  [$idx/$($subsystems.Count)] $sub ($($subData['Count']) docs)..." -ForegroundColor DarkCyan
